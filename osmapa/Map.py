...
class Map:
    ...
    def compile(self):
        osmapa.compile.produce(bin_dir=self.bin_dir, root_dir=self.root_dir, map_work_dir=self.map_work_dir, typfile=self.typfile, style=self.style, configfile=self.configfile, fid=self.fid, src_dir=self.src_dir, map_version=self.map_version, publisher_id=self.publisher_id, map_name=self.map_name, out_dir=self.out_dir, coastlinefile=self.coastlinefile, bounds_dir=self.bounds_subdir, lowercase=self.lowercase, codepage=self.codepage, verbose=self.verbose)
