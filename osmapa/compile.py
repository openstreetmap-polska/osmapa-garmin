...
def main(bin_dir, mapa_root, tmp_mapa_glowna, typfile_glowna, styl_mapy_glowna, fid_glowna, tmp_dane_osm, wersja_mapy, mapy_gotowe):
    ...
    if platform.system() == 'Windows':
        ret = os.system('start /low /b /wait java -enableassertions -Xmx6000m -jar {binarki}/mkgmap.jar --verbose --family-name=OSMapaPL --description=OSMapaPL --series-name=OSMapaPL  --coastlinefile={dane_osm}/coastlines_europe-latest.osm.pbf  --read-config={mapa_root}/config/osmapa.config --bounds={dane_osm}/bounds --family-id={fid_glowna} --product-id={fid_glowna} --mapname=66{fid_glowna}001 --overview-mapname=66{fid_glowna}000   --style-file={binarki}/resources/styles/ --style={styl}  --check-styles  -c template.args  style.typ'.format(
            mapa_root=mapa_root, binarki=bin_dir, styl=styl_mapy_glowna, fid_glowna=fid_glowna, dane_osm=tmp_dane_osm))
    elif platform.system() == 'Linux':
        ret = os.system('java -enableassertions -Xmx6000m -jar {binarki}/mkgmap.jar --verbose --family-name=OSMapaPL --description=OSMapaPL --series-name=OSMapaPL  --coastlinefile={dane_osm}/coastlines_europe-latest.osm.pbf  --read-config={mapa_root}/config/osmapa.config --bounds={dane_osm}/bounds --family-id={fid_glowna} --product-id={fid_glowna} --mapname=66{fid_glowna}001 --overview-mapname=66{fid_glowna}000   --style-file={binarki}/resources/styles/ --style={styl}  --check-styles  -c template.args  style.typ'.format(
            mapa_root=mapa_root, binarki=bin_dir, styl=styl_mapy_glowna, fid_glowna=fid_glowna, dane_osm=tmp_dane_osm))
    else:
