...
def get_osm_data_test(bin_dir, dest_dir, pbf_filename):
    ...
    print('{binarki}\\wget.exe -c http://download.geofabrik.de/{pbf_file} -t 5  -O {dane_osm}/{pbf_file}'.format(
...
def get_osm_data(bin_dir, dest_dir, pbf_filename):
    ...
    if platform.system() == 'Windows':
        ret = os.system('{binarki}\\wget.exe -c http://download.geofabrik.de/{pbf_file} -t 5  -O {dane_osm}/{pbf_file}'.format(
        binarki=bin_dir, pbf_file=pbf_filename, dane_osm=dest_dir))
    elif platform.system() == 'Linux':
        ret = os.system('wget -c http://download.geofabrik.de/{pbf_file} -t 5  -O {dane_osm}/{pbf_file}'.format(
